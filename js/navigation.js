...
function buttonClicked(button){
  if (button.classList.contains("menu")) menuClicked(button)
  if (button.classList.contains("skills")) skillsClicked(button)
  if (button.classList.contains("exps")) expsClicked(button)
  if (button.classList.contains("hobbies")) hobbiesClicked(button)
}

function menuClicked(menu){
  var i = 0
  var nodeList = [];
  nodeList[0] = menu.parentElement.querySelectorAll('button')[3]
  nodeList[1] = menu.parentElement.querySelectorAll('button')[2]
  nodeList[2] = menu.parentElement.querySelectorAll('button')[1]
  nodeList.forEach(button => {
    if (button.classList.contains('menu')) return
    button.classList.toggle('display')
  });  
}

function expsClicked(exps){
  rotateX10();
}

function mouseEntered(buttonClass){
  var nav = document.querySelector("nav");
  var connector = nav.querySelector("div."+buttonClass)
  if (connector != null){
    connector.classList.add("connectorIsHovered")
    connector.classList.add('display')
  }
}

function mouseLeft(buttonClass){
  var nav = document.querySelector("nav");
  var connector = nav.querySelector("div."+buttonClass)
  if (connector != null){
    connector.classList.remove("connectorIsHovered")
    connector.classList.remove('display')
  }
}
